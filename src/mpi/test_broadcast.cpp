...
int main(int argc, char** argv) {
    ...
    posix_memalign((void**)&ctx.buff, 4096, ctx.count * sizeof(int));
    ...
    printf("rank %d: start ibcast with count %d, root: %d\n", ctx.rank, ctx.count, ctx.root);
    ...
    printf("rank %d: start bcast with count %d, root: %d\n", ctx.rank, ctx.count, ctx.root);
